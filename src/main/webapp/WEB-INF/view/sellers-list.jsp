...
<div class="container">
    ...
    <nav class="navbar fixed-top navbar-expand-lg navbar navbar-light" style="background-color: #ffbb48;">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ">
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath}/seller/addSellerForm">Add Seller<span
            ...
            <!--  add a search box -->
            <form:form cssClass="form-inline my-2 my-lg-0 m-auto" action="searchSeller" method="GET">
                <div class="input-group">
                    <input class="form-control mr-sm-2" type="search" placeholder="Search Seller" name="searchSeller"
                           aria-label="searchSeller">
                    <button class="btn btn-outline-secondary my-2 my-sm-0" type="submit">Search Seller</button>
                </div>
            </form:form>
    ...
    <h2 class="back-color">
        Sellers
        <img src="${pageContext.request.contextPath}/resources/img/seller.svg" width="20" height="20" alt=""></h2>
    <hr>
    <table class="table table-bordered text-center">
        ...
        <c:forEach var="tempSellers" items="${sellers}">
            ...
            <c:url var="updateLink" value="/seller/updateSeller">
                <c:param name="sellerId" value="${tempSellers.id}"/>
            </c:url>

            <c:url var="deleteLink" value="/seller/deleteSeller">
                <c:param name="sellerId" value="${tempSellers.id}"/>
            </c:url>

            <tr class="table-light">
                <td>${tempSellers.id}</td>
                <td>${tempSellers.firstName}</td>
                <td>${tempSellers.lastName}</td>
                <td>${tempSellers.phone}</td>
                <td>${tempSellers.address}</td>
                <td>${tempSellers.email}</td>
                <td>${tempSellers.dateOfBirth}</td>
                <td>${tempSellers.gender}</td>
                <td>${tempSellers.registrationDate}</td>
                <td>
                    <div class="btn-group" role="group" aria-label="Basic example">
                        ...
                        <a class="btn  btn-outline-danger btn-sm" href="${deleteLink}"
                           ...
                           onclick="if (!(confirm('Are you sure you want to delete this seller?'))) return false"
                        >Delete</a>
                    </div>
                </td>
            </tr>
